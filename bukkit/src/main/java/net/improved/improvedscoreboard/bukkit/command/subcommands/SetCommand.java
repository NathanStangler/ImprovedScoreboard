...
public class SetCommand implements ScoreboardSubcommand {
    @Override
    public boolean execute(@NotNull CommandSender sender, @NotNull String[] args) {
        if (args.length < 4) {
            Message.HELP_SET.send(sender);
            return false;
        }
        ...
        ScoreboardComponent component = scoreboard.getData().getLine(line);
        if (component instanceof AnimatedScoreboardComponent animated) {
            animated.setText(List.of(text));
        } else {
            ((StandardScoreboardComponent) component).setText(text);
        }
        Message.SET_SUCCESS.send(sender, line, name, text);
        return true;
    }
